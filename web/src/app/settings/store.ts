...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  setFullRefreshAccountIntervalMinute: (value) => {
    set((state) => {
      if (!state.config) {
        return {};
      }
      return {
        config: {
          ...state.config,
          full_refresh_account_interval_minute: value,
        },
      };
    });
  },
  ...
  setRegisterFlaresolverrUrl: (value) => {
    set((state) => state.registerConfig ? { registerConfig: { ...state.registerConfig, flaresolverr_url: value } } : {});
  },
